...
*/
/*
This class connects to a serial port socket server, and receives data.
...
exports.ArmSensorArduinoSocket = new Class({
	...
	initialize: function(NET_PORT, options){		
		...
		if (!NET_PORT) console.log("You must specify a net port.");
...
//        if (this.DEBUG) this.on('newLowerValue', function(){console.log("new lower value")});
    ...
    createNewNetConnection: function(){
        try{
            this.serialPort = new net.Stream();
            this.serialPort.connect(this.port, '127.0.0.1', function() {
                console.log('ARDUINO TCP CONNECTED TO: ' + '127.0.0.1' + ':' + this.port);
			}.bind(this));
            
			

            this.serialPort.on('data', this.serialData.bind(this));
            this.serialPort.on('connect', this.open.bind(this)); //this does not seem to get called by lib
...
//       this.updateTimeDataReceived();
	...
	open: function(){ console.log('Arduino Opened');},
