 ...
 (function(){
	...
	if(namespace.BlobCircle === undefined) {
		...
		*/
		...
		BlobCircle.DAMPING = 0.29;BlobCircle.TRACKING_FORCE_ENVELOPE = 4.3999999999999995;BlobCircle.NEIGHBOR_FORCE_ENVELOPE = 2.1;BlobCircle.BEAT_FORCE = 75;BlobCircle.BEAT_FORCE_RANDOM = 14;
		...
		p._init = function() {
			...
			this._radiusAnimation = {"radius": -1};
		...
		p.changeRotationOffset = function(aOffset, aRotationOffset) {
			...
			new TWEEN.Tween(this).to({"_centerOffset": aOffset, "_centerRotationOffset": aRotationOffset}, 300).easing(TWEEN.Easing.Quadratic.EaseOut).start();
		};
		...
		p.animateRadius = function(aRadius, aTime, aEasing) {
			...
			this._radiusTween.to({"radius": aRadius}, 1000*aTime).easing(aEasing).onUpdate(this._radiusAnimationUpdateCallback).start();
		};
