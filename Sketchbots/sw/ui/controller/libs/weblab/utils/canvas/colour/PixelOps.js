(function(){
    ...
    if (namespace.PixelOps === undefined)
	{
		...
		{		
			...
			{			
				...
				if (pix[i+2] == 0) // where BLUE is 0...
				{
					...
					var bounds = this.floodFill(coords, {r:0, g:0, b:b});					
					...
					if (bounds.pixels.length > params.speckSize)
					{
						...
						// store / draw
						this.drawBlobBoundsRect(bounds, {r:255, g:0, b:0, a:.8});
		...
		p.createBlobInDOM = function(bounds)
		{
			...
			{
				this.colourPixel(pix[i]-offset, {r:255, g:0, b:0}, tempData);
			}
		...
		p.floodFill = function(pixel, col)
		{
			...
			this.startColour = {r:data[pixelPos], g:data[pixelPos+1], b:data[pixelPos+2]};
			
			var extremes = [{x:pixel.x, y:pixel.y}, {x:0, y:0}];
			var pixels = [];
