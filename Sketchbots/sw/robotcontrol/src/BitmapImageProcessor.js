...
exports.BitmapImageProcessor = new Class({
	...
	//////////////////////////////////////////////////////////////////////////////////////////			
		...
		var polyResult = this.bezierTracer.getPolyPaths(ctx1,[{x:0, y:0, w:1000, h:1000}]);
	...
	removeEggPoints: function(paths, ctx){
		...
		var edgePoints = SimpleTrig.getPointsOnACircle(ctx.canvas.width/2, {x:ctx.canvas.width/2, y:ctx.canvas.width/2}, 200);
		...
		for (var i=0, n=paths.length; i<n; ++i)
		{
			...
			for (var j=0, m=path.length; j<m; ++j)
			{
				...
				if (keep)
				{
					var newPoint = {x:paths[i][j].x, y:paths[i][j].y};
	...
	_processSource_adaptThresholds: function(canvas, s, t){
		...
		this.cacheDataArray = [];
	    for (var i in this.cacheData) { this.cacheDataArray[i] = this.cacheData[i]; }
		...
		for (var i in outData) { imgDataData[i] = outData[i]; }
